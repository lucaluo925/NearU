...
/** Ambient idle line for guest users, based on history. */
function guestIdleMsg(guestBond: number, guestActions: number, gapSinceActionMs: number): string {
  ...
  if (guestActions >= 5) return pick(["you've been finding good stuff", "nice picks lately 🐾"])
...
function SpeechBubble({ message, fading }: { message: string; fading: boolean }) {
  return (
    <div
      className={`relative max-w-[280px] self-end ${fading ? 'pet-speech-out' : 'pet-speech-in'}`}
      aria-live="polite"
    >
...
interface RecItem { id: string; title: string; category: string }
const REC_SESSION_KEY = 'nearu-pet-rec-3'
interface RecCache { items: RecItem[]; label: string }

function PetRecommendation({ onClose }: { onClose: () => void }) {
  const [items, setItems]     = useState<RecItem[]>([])
  const [loading, setLoading] = useState(true)
  const [label, setLabel]     = useState<string>("you might like these")
  ...
  useEffect(() => {
    try {
      ...
      if (cached) {
        ...
        if (parsed?.items?.length > 0) {
          setItems(parsed.items); setLabel(parsed.label ?? "you might like these")
          setLoading(false); return
        }
      }
    } catch {}

    const catSignal = loadCatSignal()
    ...
    fetch('/api/items?limit=60&sort=recent')
      ...
      .then((all: RecItem[]) => {
        ...
        // Score by category signal frequency, shuffle within same score for variety
        const scored = candidates
          .map((i) => ({
            item:  i,
            score: catSignal.filter((c) => c === i.category).length,
            rand:  Math.random(),
          }))
          .sort((a, b) => b.score - a.score || b.rand - a.rand)

        const top       = scored.slice(0, 3).map((s) => s.item)
        const hasMatch  = top.some((i) => catSignal.includes(i.category))
        const resolved  = hasMatch ? pick([...LABEL_MATCHED]) : pick([...LABEL_FALLBACK])

        setItems(top); setLabel(resolved)
        try { sessionStorage.setItem(REC_SESSION_KEY, JSON.stringify({ items: top, label: resolved } as RecCache)) } catch {}
      })
  ...
  if (loading) return (
    <div className="px-4 pb-3">
      ...
      <div className="flex flex-col gap-1.5">
        {[0, 1, 2].map((i) => <div key={i} className="h-9 rounded-xl skeleton" />)}
      </div>
    </div>
  )
...
export default function PetWidget() {
  ...
  useEffect(() => {
    function handler(e: Event) {
      ...
      if (isLoggedOutRef.current) {
        // Guest: bump guest bond on any action event and show a simple reaction
        if (detail.action) {
        ...
        // Guest dialogue: simplified bond messages (no pet personality needed)
        let msg: string | null = null
        if (detail.action === 'save')     msg = pick(["good find.", "noted.", "saved."])
        else if (detail.action === 'share')    msg = pick(["shared.", "spreading the word 👀"])
        else if (detail.action === 'calendar') msg = pick(["added.", "don't miss it 🗓️"])
      ...
      if (msg) say(maybePersonalize(msg, userNameRef.current, bondRef.current))
  ...
  function handlePetClick() {
    ...
    if (isLoggedOut) {
      // Guest: bump bond, pick bond-aware message
    ...
    >
      ...
      {open && (
        <div className="relative w-[260px] max-w-[calc(100vw-2rem)] bg-white border border-[#E5E7EB] rounded-2xl shadow-2xl overflow-hidden pet-card-open">
          ...
          {/* Inline pet voice */}
          {message && !namingMode && (
            <div className="mx-4 mb-3 px-3 py-2.5 bg-[#F9FAFB] border border-[#F3F4F6] rounded-xl animate-fade-up">
              <p className="text-[12px] text-[#374151] leading-relaxed">"{message}"</p>
            </div>
          )}
          ...
          {/* Recommendation — shown to everyone */}
          <div className="border-t border-[#F3F4F6] mt-1 pt-3">
            <PetRecommendation onClose={() => setOpen(false)} />
          </div>
          ...
          {!isGuest ? (
            ...
            <div className="border-t border-[#F3F4F6] px-4 py-3">
              ...
              >
                ...
                <span>
                  {guestBond >= 8
                    ? 'Sign in so I can really remember you'
                    : guestBond >= 4
                    ? "Sign in to keep your companion's progress"
                    : guestBond >= 2
