 ...
 * Shared between the Next.js cron route and the Node.js ingest scripts.
 */
...
/**
 * Resolve a raw location name into a specific campus address + coordinates.
...
export function resolveCampusLocation(
  ...
  const normalized = rawName
    .toLowerCase()
    .replace(/\brm\.?\s*\d+\w*\b/g, '')
    .replace(/\broom\s+\d+\w*\b/g, '')
    .replace(/\b\d{3,4}[a-z]?\b/g, '')
    .replace(/\buc davis\b/gi, '')
    .replace(/[^a-z0-9&\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
